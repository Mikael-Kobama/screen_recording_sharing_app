...
const validateWithArcjet = async (fingerPrint: string) => {
  const rateLimit = aj.withRule(
    fixedWindow({
      ...
      window: "1m",
      max: 2,
      characteristics: ["fingersprint"],
    }),
  );
  const req = await request();
  const decision = await rateLimit.protect(req, { fingersprint: fingerPrint });
...
const buildVideoWithUserQuery = () =>
  db
    ...
    .from(videos)
    .leftJoin(user, eq(videos.user.id));
...
export cont getThumbnailUploadUrl = withErrorHandling(
